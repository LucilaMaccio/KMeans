...
# Change Max Reursion limit because we exceed de limit recursion default with the dendrogram
...
for i in range(0, len(X)):
    ...
    if element == None:
        ...
        if len(element) != 11:
            ...
            if location.find("Palacio") != -1 or location.find("Embajadores") != -1 or location.find(
                    "Cortes") != -1 or location.find("Justicia") != -1 or location.find(
                    "Universidad") != -1 or location.find("Sol") != -1:
            ...
            elif location.find("Imperial") != -1 or location.find("Acacias") != -1 or location.find(
                    "Chopera") != -1 or location.find("Legazpi") != -1 or location.find(
                    "Delicias") != -1 or location.find("Palos de Moguer") != -1 or location.find("Atocha") != -1:
            ...
            elif location.find("Pacífico") != -1 or location.find("Adelfas") != -1 or location.find(
                    "Estrella") != -1 or location.find("Ibiza") != -1 or location.find(
                    "Jerónimos") != -1 or location.find("Niño Jesús") != -1:
            ...
            elif location.find("Recoletos") != -1 or location.find("Goya") != -1 or location.find(
                    "Fuente del Berro") != -1 or location.find("Guindalera") != -1 or location.find(
                    "Lista") != -1 or location.find("Castellana") != -1:
            ...
            elif location.find("Gaztambide") != -1 or location.find("Arapiles") != -1 or location.find(
                    "Trafalgar") != -1 or location.find("Almagro") != -1 or location.find(
                    "Rios Rosas") != -1 or location.find("Vallehermoso") != -1:
...
#dendogram = sch.dendrogram(sch.linkage(fullArray, method='ward'))
"""
...
# Debido a que es dificil ver el numero de cluster, debido al tamaño de los datos,
# Parece ser que el numero de cluster optimo es 3

hc = AgglomerativeClustering(n_clusters=3,
                    affinity='euclidean',
                    linkage='ward')

pred_y = hc.fit_predict(fullArray)

print("Aglomerant for n_clusters = 3")
print(pred_y)

# if score is near to 1, the clustering is good
silhouetteScore = silhouette_score(fullArray, hc.labels_, metric='euclidean')
print(silhouetteScore)

# if score is near to 0, the clustering is good
dbScore = davies_bouldin_score(fullArray, hc.labels_)
print(dbScore)
...
# -----------------------------------------------------------------------------
# Elbow method to get best number of clusters
dendogram = sch.dendrogram(sch.linkage(filteredArray, method='ward'))
...
pred_y = hc.fit_predict(filteredArray)

print("Aglomerant for n_clusters = 3")
print(pred_y)

# if score is near to 1, the clustering is good
silhouetteScore = silhouette_score(filteredArray, hc.labels_, metric='euclidean')
print(silhouetteScore)

# if score is near to 0, the clustering is good
dbScore = davies_bouldin_score(filteredArray, hc.labels_)
print(dbScore)
